...
type VoteRow = { caption_id: string; profile_id: string; vote_value: number };
...
export default async function CaptionsPage() {
  ...
  const filtered = data ?? [];
  const captionIds = filtered.map((c) => c.id);
  const imageIds = Array.from(new Set(filtered.map((c) => c.image_id).filter((id): id is string => Boolean(id))));

  const { data: votes } = captionIds.length > 0
    ? await supabase.from("caption_votes").select("caption_id,profile_id,vote_value").in("caption_id", captionIds)
    : { data: [] as VoteRow[] };
  ...
  const voteTotals = new Map<string, { up: number; down: number; currentUserVote: 1 | -1 | 0 }>();
  for (const caption of filtered) voteTotals.set(caption.id, { up: 0, down: 0, currentUserVote: 0 });
  (votes ?? []).forEach((vote) => {
    const entry = voteTotals.get(vote.caption_id);
    if (!entry) return;
    if (vote.vote_value === 1) entry.up += 1;
    if (vote.vote_value === -1) entry.down += 1;
    if (vote.profile_id === user.id) entry.currentUserVote = vote.vote_value === 1 ? 1 : -1;
  });
      ...
      <div className="mx-auto w-full max-w-5xl space-y-10">
        ...
        <header className="space-y-2">
          ...
          <p className="max-w-2xl text-base sm:text-lg" style={{ color: "var(--muted)" }}>
            Rate public captions with an up-vote or down-vote.
          </p>
        </header>
        ...
        {error ? (
          ...
          <CaptionVoteCarousel
            captions={filtered.map((caption) => {
              const totals = voteTotals.get(caption.id) ?? { up: 0, down: 0, currentUserVote: 0 as 0 };
              return {
                id: caption.id,
